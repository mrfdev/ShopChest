...
/**
 * Client-side text and presentation settings for a ShopChest hologram panel.
 *
...
public record TextDisplayData(Component text, int lineWidth, int backgroundColor, boolean fixedFacing, float scale) {
    ...
    public TextDisplayData(String text) {
        this(fromLegacy(text));
    }

    public TextDisplayData(String text, int lineWidth, int backgroundColor, boolean fixedFacing, float scale) {
        this(fromLegacy(text), lineWidth, backgroundColor, fixedFacing, scale);
    }

    public TextDisplayData(String text, int lineWidth, int backgroundColor, boolean fixedFacing) {
        this(text, lineWidth, backgroundColor, fixedFacing, DEFAULT_SCALE);
    }

    private static Component fromLegacy(String text) {
        return TextComponentHelper.LEGACY_COMPONENT_SERIALIZER.deserialize(text == null ? "" : text);
    }
}
